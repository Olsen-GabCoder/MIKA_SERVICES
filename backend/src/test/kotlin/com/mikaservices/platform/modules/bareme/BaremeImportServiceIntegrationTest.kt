...
class BaremeImportServiceIntegrationTest {
    ...
    fun `import Excel minimal - une feuille corps d'etat`() {
        ...
        try {
            ...
            sheet0.getRow(3).createCell(2).setCellValue("Sable")
            sheet0.getRow(3).createCell(3).setCellValue("m3")
            sheet0.getRow(3).createCell(4).setCellValue(12500.0)
            sheet0.getRow(3).createCell(6).setCellValue("Transp")
