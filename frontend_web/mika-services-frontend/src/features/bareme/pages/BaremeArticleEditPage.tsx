...
import { useBaremeArticle, useCorpsEtat, useUpdateBaremeArticle } from '../hooks/useBaremeQueries'
...
export function BaremeArticleEditPage() {
  ...
  useEffect(() => {
    ...
    setPrixEstime(article.prixParFournisseur?.some((p) => p.prixEstime) ?? false)
    if (article.type === TypeLigneBareme.MATERIAU) {
      const firstPf = article.prixParFournisseur?.find((p) => p.prixTtc != null) ?? article.prixParFournisseur?.[0]
      setFournisseurNom(firstPf?.fournisseurNom ?? '')
      setFournisseurContact(firstPf?.fournisseurContact ?? '')
      setPrixTtc(firstPf?.prixTtc != null ? String(firstPf.prixTtc) : '')
      setDatePrix(firstPf?.datePrix ?? '')
    } else {
  ...
  if (!articleId || Number.isNaN(articleId)) {
    return <PageContainer size="full" className="min-h-full bg-gray-50/80 dark:bg-gray-900/80"><p className="text-red-600">{t('detail.invalidId')}</p></PageContainer>
  }

  if (isLoading || !article) {
    return <PageContainer size="full" className="min-h-full bg-gray-50/80 dark:bg-gray-900/80"><div className="h-10 w-56 rounded bg-gray-200 dark:bg-gray-700 animate-pulse" /></PageContainer>
  }
    ...
    <PageContainer size="full" className="min-h-full bg-gray-50/80 dark:bg-gray-900/80 space-y-6">
      <div className="rounded-2xl bg-gradient-to-br from-primary to-primary-dark text-white shadow-lg overflow-hidden">
        <div className="px-6 py-6 md:py-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold">{t('edit.title')}</h1>
            <p className="text-white/90 text-sm mt-1">{t('edit.subtitle')}</p>
          </div>
          <Button type="button" variant="outline" className="!border-white/40 !text-white hover:!bg-white/15" onClick={() => navigate(`/bareme/articles/${articleId}`)}>
            ← {t('detail.backToList')}
          </Button>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-600 p-6 shadow-sm">
        {error && <div className="mb-5 rounded-lg border border-red-200 dark:border-red-700 bg-red-50 dark:bg-red-900/30 px-4 py-3 text-sm text-red-700 dark:text-red-200">{error}</div>}
          ...
          <div className="rounded-xl border border-gray-200 dark:border-gray-600 p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <select value={type} disabled onChange={(e) => setType(e.target.value as TypeLigneBareme)} className={inputClass}>
              ...
              <div className="space-y-3">
                ...
                {lignes.map((ligne, idx) => (
                  <div key={idx} className="grid grid-cols-1 md:grid-cols-6 gap-2 items-start">
                    <input placeholder={t('detail.colLibelle')} value={ligne.libelle} onChange={(e) => setLignes((prev) => prev.map((l, i) => i === idx ? { ...l, libelle: e.target.value } : l))} className={`${inputClass} md:col-span-2`} />
                    ...
                    <button type="button" onClick={() => setLignes((prev) => prev.filter((_, i) => i !== idx))} className="px-3 py-2 border rounded-lg text-red-600 border-red-200 hover:bg-red-50 dark:border-red-700 dark:hover:bg-red-900/20">{t('create.removeLine')}</button>
                  </div>
                ))}
                <button type="button" onClick={() => setLignes((prev) => [...prev, emptyLigne()])} className="px-3 py-2 rounded-lg bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-sm">{t('create.addLine')}</button>
              </div>
            </div>
          )}

          <div className="flex items-center gap-3 pt-2 border-t border-gray-200 dark:border-gray-600">
            <Button type="submit" disabled={!isValid || updateMutation.isPending} isLoading={updateMutation.isPending}>
              {updateMutation.isPending ? t('edit.saving') : t('edit.save')}
            </Button>
            <Button type="button" variant="outline" onClick={() => navigate(`/bareme/articles/${articleId}`)}>
              {t('create.cancel')}
            </Button>
          </div>
        </form>
