...
export const Header = () => {
  ...
  return (
    <header className="relative shrink-0 isolate overflow-hidden h-[var(--layout-header-height,4.5rem)]" style={{ minHeight: 'var(--layout-header-height, 4.5rem)' }}>
      {/* Fond légèrement structuré (évite le bloc blanc plat) */}
      ...
      <div className="relative h-full flex items-center justify-between gap-6 px-6 md:px-8">
        {/* Gauche : logo + tagline */}
        <Link
          to="/"
          className="group flex items-center gap-4 min-w-0 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/25 focus:ring-offset-2 dark:focus:ring-offset-gray-900"
          aria-label={t('header.homeAria')}
        >
          <img
            src="/Logo_mika_services.png"
            alt=""
            className="h-12 w-auto object-contain transition-all duration-200 group-hover:opacity-90 group-active:opacity-95"
          />
        ...
        <div className="flex items-center gap-4 min-w-0 max-w-[60%] md:max-w-[50%]">
